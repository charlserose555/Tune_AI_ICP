...
export const getName = (activeSports) => {
    /* eslint-disable */
...
export const getMarkets = (event, activeSports) => {
...
export const getIsLock = ({ item, sports, isLive, event }) => {
    /* eslint-disable */
    if (!sports) {
        return false;
    }
...
export const checkUpdate = (params, matchs, sport_id, setSportsMatchs) => {
    if (matchs.length) {
...
export const convertHandicap = (hc, IF) => {
...
export const convertBetslipData = ({ event, odd, oddType, sports }) => {
    let oddName = '';
    const oddId = odd.id;
...
export const addRemoveBetslip = (betslipData, betslip) => {
...
export const checkActive = (betslipData, id, oddType) =>
    betslipData.find((e) => e.oddId === id && e.oddType === oddType) !== undefined;

export const abbreviate = (value, decimal = 2) => {
